...
* Experimental support for Expert Parallelism (EP), which allows experts in Mixture-of-Experts (MoE) models to be deployed on separate GPUs.
* FP8 KV cache is supported to reduce memory footprint by allowing approximately double the amount of space for KV cache allocation. This enables either processing longer context lengths for individual requests, or handling more concurrent request batches.
* More multi-modality models are supported with image/video as input, like whisper-large-v3, Qwen3-VL-30B-A3B-Instruct, DeepSeek-OCR and PaddleOCR-VL, etc.
...
* Expert Parallelism
  ...
  Experimental support for [Expert Parallelism](https://docs.vllm.ai/en/stable/serving/expert_parallel_deployment), which allows experts in Mixture-of-Experts (MoE) models to be deployed on separate GPUs. It typically works together with Data Parallelism.
...
| Multi Modality  | zai-org/GLM-4.1V-9B-Thinking              |✅︎|✅︎| |
| Multi Modality  | zai-org/GLM-4.5V                          |✅︎|✅︎| |
| Multi Modality  | Kwai-Keye/Keye-VL-1.5-8B                  |✅︎|✅︎| |
| Multi Modality  | Kwai-Keye/Keye-VL-8B-Preview              |✅︎|✅︎| |
...
* Server hang issue in some scenarios and may can workaround by adjusting settings like `max_num_batched_tokens`, `tp`.
...
1. Get the released docker image with command `docker pull intel/vllm:0.12.0-xpu`
2. Instantiate a docker container with command `docker run -t -d --shm-size 10g --net=host --ipc=host --privileged -v /dev/dri/by-path:/dev/dri/by-path --name=vllm-test --device /dev/dri:/dev/dri --entrypoint= intel/vllm:0.12.0-xpu /bin/bash`
