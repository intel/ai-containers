        ...
        for each_section in config.sections():
            ...
            for each_key, each_val in config.items(each_section):
                ...
                if each_key == "support-accuracy":
                    if each_val is not None:
                        if (
                            ast.literal_eval(each_val) is False
...
class TFTimelinePresenter:
    ...
    def merge_two_csv_files_v2(self, merged_filepaths, a, b, tags=["stock", "intel"]):
        ...
        merged = merged.rename(
            columns={
                merged.columns.values[1]: tags[0],
                merged.columns.values[4]: tags[1],
            }
        )
        ...
        merged = merged.drop(
            columns=[
                merged.columns.values[2],
                merged.columns.values[3],
                merged.columns.values[5],
                merged.columns.values[6],
            ]
        )
        if self.showAbsNumber is False:
            ret = merged.drop(
                columns=[merged.columns.values[1], merged.columns.values[4]]
            )
        else:
            ret = merged
        ...
        self.create_csv_among_extra_common_ops(
            extra_a, common_a, fpath, tags[0], tags[1]
        )

        fpath = merged_filepaths[2]
        self.create_csv_among_extra_common_ops(
            extra_b, common_b, fpath, tags[1], tags[0]
        )
    ...
    def plot_compare_ratio_bar_charts(
        self, fpath, tags=["stock", "intel"], num_hotspots=20, max_speedup=100
    ):
        import csv
...
class oneDNNUtils:
    ...
    def stats_comp(
        self, name, Type, onednn_log1, onednn_log2, n=50, tags=["run1", "run2"]
    ):
        import matplotlib.pyplot as plt
        import numpy as np
        import pandas as pd
        ...
        if Type == "count":
            jitstat = pd.concat(
                (d1[name].value_counts(), d2[name].value_counts()), axis=1, sort=True
            )
            ...
            jitstat = pd.concat(
                (d1.groupby(name)["time"].sum(), d2.groupby(name)["time"].sum()),
                axis=1,
                sort=True,
            )
            jitstat.columns = ("1-" + log1, "2-" + log2)
    ...
    def parse_raw_output_to_csv(
        self, filepath, csvpath="mkldnn_log.csv", keyword="dnnl_verbose"
    ):
        # filepath = 'Iliad.txt'
        import csv
