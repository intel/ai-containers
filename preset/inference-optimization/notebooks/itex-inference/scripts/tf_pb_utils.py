...
class TF_PB_Utils:
    ...
    def parse_pb_in_savemodel(self, folderpath):
        ...
        with tf.compat.v1.Session(graph=graph) as sess:
            tf.compat.v1.saved_model.loader.load(
                sess, [tf.compat.v1.saved_model.tag_constants.SERVING], folderpath
            )
        return graph
    ...
    def breakdown(self, data, columnindex, group, topk=5, keyword="", rowindex=-1):
        ...
        if rowindex != -1:
            print(
                "\n Group ops on row: {0} , {1} ".format(
                    rowindex, groupdata.index[rowindex]
                )
            )
            topo_data = data.loc[data[group] == groupdata.index[rowindex]]
            sorted_topo_data = (
                topo_data.groupby("op_type").size().sort_values(ascending=False)
            )
    ...
    def dump_columns(self, data, topk=10):
        for i in range(5):
            ...
            group = self.fnames[i]
            groupdata = (
                data.groupby(group).size().sort_values(ascending=False).head(topk)
            )
            print(groupdata)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "file",
        type=str,
        help="The file name of the frozen graph/ The folder name contained the saved model",
    )
    ...
    topo_data_list, group_list = tfpb.breakdown(
        data, args.column, tfpb.fnames[args.column], rowindex=args.row
    )
    if args.row == -1:
        sys.exit()
