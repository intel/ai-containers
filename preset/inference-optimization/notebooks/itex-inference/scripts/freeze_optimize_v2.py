...
from tensorflow.python.framework import convert_to_constants, graph_util, importer
...
from tensorflow.python.tools import optimize_for_inference_lib, saved_model_utils
...
def run_main(unused_args):
    ...
    if FLAGS.saved_model_tags == "":
        ...
        if not found:
            print(
                "Supplied tags",
                tag_set,
                "is not in available tag set,\
                    please use one or more of these",
                avail_tags,
                "Using --saved_model_tags",
            )
            exit(1)
    ...
    if sig_key not in list(pretrained_model.signatures.keys()):
        print(
            sig_key,
            "is not in ",
            list(pretrained_model.signatures.keys()),
    ...
    frozen_func = convert_to_constants.convert_variables_to_constants_v2(
        infer, lower_control_flow=True
    )

    frozen_func.graph.structured_outputs = nest.pack_sequence_as(
        infer.graph.structured_outputs, frozen_func.graph.structured_outputs
    )
    ...
    with session.Session() as sess:
        ...
        signature_inputs = {
            (tensor.name.split(":"))[0]: model_utils.build_tensor_info(tensor)
            for tensor in inputs
        }
        signature_outputs = {
            name: model_utils.build_tensor_info(souts[name]) for name in souts
        }
        ...
        builder.add_meta_graph_and_variables(
            sess, tags=[tag_constants.SERVING], signature_def_map=signature_def_map
        )
        builder.save()
