...
def should_distribute():
    return dist.is_available() and int(os.environ['WORLD_SIZE']) > 1
...
def main():
    ...
    if should_distribute():
        print('Using distributed PyTorch with {} backend'.format('CCL'))
        dist.init_process_group(backend=dist.Backend.CCL)
    ...
    if is_distributed():
        Distributor = nn.parallel.DistributedDataParallel if args.device=='xpu' \
