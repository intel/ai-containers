...
FROM python-base AS classical-ml-notebook
...
COPY --chown=1001:0 ml-requirements.txt tmp-requirements.txt
...
RUN python -m ipykernel install --prefix=${APP_ROOT} --name 'classical-ml' --display-name='classical-ml' && \
