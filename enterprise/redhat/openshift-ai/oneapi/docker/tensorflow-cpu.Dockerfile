...
FROM python-base AS tensorflow-base
...
COPY --chown=1001:0 tensorflow-requirements.txt tmp-requirements.txt
...
RUN python -m ipykernel install --prefix=${APP_ROOT} --name 'tensorflow-cpu' --display-name='tensorflow-cpu' && \
